...
{
    public $method = 'get';
    public $auto_authenticate = true;
    public $strict_cookie_auth = true;
    ...
    public function _clear_datamapper_cache()
    {
    ...
    public function add_to_history($message)
    {
        $h = new History();
        $h->message = $message;
    ...
    public function authenticate($require_king = false)
    {
        $token = false;
        $cookie = false;
        ...
        } elseif (isset($_SERVER['HTTP_X_KOKEN_TOKEN'])) {
    ...
    public function _compile_plugins()
    {
    ...
    public function check_for_rewrite()
    {
        ...
        if (!function_exists('_sort')) {
            function _sort($one, $two)
            {
                if ($one['featured'] && !$two['featured']) {
                    return -1;
                } elseif ($one['featured'] && $two['featured']) {
