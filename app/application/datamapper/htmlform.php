...
class DMZ_HTMLForm
{
     ...
     */
    public function render_field($object, $field, $type = null, $options = null)
    {
        $value = '';
        ...
        } elseif (function_exists('input_' . $type)) {
    ...
    public function _render_row_from_form($object, $field, $options, $row_template, $row = true)
    {
        if (is_int($field)) {
    ...
    public function _get_type($object, $field, $type)
    {
        if (empty($type)) {
    ...
    public function _get_validation_rule($object, $field, $rule)
    {
        $r = $this->_get_validation_info($object, $field, 'rules', false);
        if ($r !== false) {
            if (isset($r[$rule])) {
                return $r[$rule];
            } elseif (in_array($rule, $r, true)) {
                return true;
            }
        }
        return false;
    ...
    public function _input_hidden($object, $id, $value, $options)
    {
        return $this->_render_simple_input('hidden', $id, $value, $options);
    }

    // render a single-line text input
    public function _input_text($object, $id, $value, $options)
    {
        return $this->_render_simple_input('text', $id, $value, $options);
    }

    // render a password input
    public function _input_password($object, $id, $value, $options)
    {
    ...
    public function _input_textarea($object, $id, $value, $options)
    {
        if (isset($options['value'])) {
            $value = $options['value'];
            unset($options['value']);
    ...
    // render a dropdown
    public function _input_dropdown($object, $id, $value, $options)
    {
        $list = $options['list'];
        unset($options['list']);
        $selected = $value;
    ...
    public function _options($list, $sel)
    {
        $l = '';
    ...
    public function _render_node($type, $attributes, $content = false)
    {
        // generate node
        $res = '<' . $type;
