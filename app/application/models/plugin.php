...
class Plugin extends DataMapper
{
    ...
    public function _get_plugin($plugins)
    {
    ...
    public function run_plugin_method($method, $plugins, $arg = null)
    {
        $plugin = $this->_get_plugin($plugins);
        if ($plugin && isset($plugin['php_class_name'])) {
    ...
    public function save_data($plugins, $data)
    {
        $plugin = $this->_get_plugin($plugins);
        ...
        if ($plugin && isset($plugin['data'])) {
            ...
            foreach ($data as $name => $val) {
                if (isset($plugin['data'][$name])) {
                    ...
                    } elseif ($info['type'] === 'text' && isset($raw_input_data[$name])) {
